...
export async function getStatus(request, res) {
  ...
  const status = statuses.findId(
    "https://".concat(config.siteHost, "/people", request.url)
  );
 ...
 */
export async function handlePOSTToCollection(
  request,
  personId,
  collectionName,
  person
  ...
  if (inboundActivity.type !== "Delete") {
    ...
    const respondingActivity = createRespondingActivity(
      remoteActorInboxURL,
      person,
      inboundActivity
    );
